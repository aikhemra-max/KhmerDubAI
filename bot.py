...
MIN_SPEED = float(os.getenv("MIN_SPEED", "0.92"))
MAX_SPEED = float(os.getenv("MAX_SPEED", "1.08"))
...
# ==============================================================================
# Voice Profiles and Emotion Configuration
# ==============================================================================
...
def get_session(chat_id: int) -> dict:
    """Retrieve or create a session for a chat."""
...
async def clear_previous_project(
    ...
    """Clear all tracked messages from a previous project."""
...
async def expire_after_inactivity(
    ...
    """Expire session if no activity for SESSION_IDLE_SECONDS."""
    while True:
        session = get_session(chat_id)
...
def touch_session(context: ContextTypes.DEFAULT_TYPE, chat_id: int) -> None:
    """Update last activity time and reset expiry timer."""
    session = get_session(chat_id)
...
async def require_new_project_started(
    ...
    """Check if user has started a new project."""
...
async def voice_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Handle /voice command to synthesize Khmer text."""
...
def count_srt_blocks(srt_text: str) -> int:
    """Count the number of SRT blocks in the text."""
...
def parse_signed_number(value: str) -> int:
    """Parse signed number from string like '+5%' or '-16Hz'."""
...
def signed_hz(value: int) -> str:
    """Format number as signed Hz value."""
    return f"{value:+d}Hz"


# ==============================================================================
# Whisper Model Management (Lazy Initialization)
# ==============================================================================
...
# ==============================================================================
# Gemini AI Integration (Lazy Initialization)
# ==============================================================================
...
def translation_prompt() -> str:
    """Generate system prompt for Khmer translation."""
    return """You are KhmerDubAI V9 Speaker Lock, an Expert Khmer Movie Subtitler,
Character Continuity Editor, and Emotional Dubbing Translator.

TASK:
Translate the complete source SRT into natural spoken Khmer for movie dubbing.

NON-NEGOTIABLE OUTPUT:
- Return ONLY valid SRT.
- No Markdown, notes, explanations, or source language text.
...
4. Infer speaker identity from names, titles, pronouns, previous lines, replies, and scene continuity.
...
STATUS AND PRONOUNS:
Choose Khmer pronouns and titles according to age, rank, relationship, historical setting, and emotional state.
...
VOICE TAGS — exactly one per line:
[M_YOUNG] [F_YOUNG] [M_ADULT] [F_ADULT] [M_OLD] [F_OLD] [BOY] [GIRL]
[M_THINK] [F_THINK] [NARRATOR_M] [NARRATOR_F]

EMOTION TAGS — exactly one per line:
[NEUTRAL] [HAPPY] [SAD] [ANGRY] [FEAR] [LOVE] [SARCASM] [CRYING] [THINKING]

VOICE CLASSIFICATION:
- Male speech → a male tag (M_YOUNG, M_ADULT, M_OLD, BOY, M_THINK, NARRATOR_M).
- Female speech → a female tag (F_YOUNG, F_ADULT, F_OLD, GIRL, F_THINK, NARRATOR_F).
...
- Valid SRT with proper structure
- Same numbering and timestamps as input
- Same block count as input
- No source language characters
- No missing or repeated dialogue
- Natural spoken Khmer
- Stable speaker identity across scene
...
def translate_to_khmer_srt(source_srt: str) -> str:
    """Translate source SRT to Khmer using Gemini."""
    expected_blocks = count_srt_blocks(source_srt)
    if expected_blocks == 0:
        raise RuntimeError("No SRT blocks found in source.")
    ...
    for attempt in range(1, TRANSLATION_RETRIES + 1):
        try:
            ...
            logger.info("Translation successful: %d blocks", actual_blocks)
            return result
        except Exception as exc:
            last_error = exc
            logger.warning(
...
def transcribe_to_srt(media_path: Path) -> str:
    """Transcribe audio to Chinese SRT using Whisper."""
    ...
    logger.info("Transcribed %d segments", len(blocks))
    return "\n\n".join(blocks) + "\n"
...
def parse_tagged_srt(srt_text: str) -> list[SubtitleCue]:
    """Parse SRT with voice and emotion tags."""
    normalized = srt_text.replace("\r\n", "\n").replace("\r", "\n").strip()
    blocks = re.split(r"\n\s*\n", normalized)
...
def validate_voice_mapping(cues: list[SubtitleCue]) -> None:
    """Validate that voice tags map to correct gender voices."""
    for cue in cues:
...
def combined_voice_settings(cue: SubtitleCue) -> dict:
    """Compute combined voice settings from profile and emotion."""
...
def validate_media_duration(media_path: Path) -> None:
    """Validate media duration is within limits."""
    duration = ffprobe_duration(media_path)
    if duration > MAX_MEDIA_SECONDS + 0.5:
        raise ValueError(
            f"វីដេអូ ឬសំឡេងវែងជាង {MAX_MEDIA_SECONDS} វិនាទី។ "
            f"សូមកាត់ឱ្យតូចជាងនេះ។"
        )
...
async def synthesize_with_retry(cue: SubtitleCue, output_path: Path) -> None:
    """Synthesize speech for a subtitle cue with retries."""
...
async def prepare_cue_audio(
    ...
    """Prepare audio for a single cue: synthesize and time-align."""
    ...
    if raw_duration > target_duration:
        ...
        safe_speed = min(MAX_SPEED, max(MIN_SPEED, desired_speed))
    else:
        safe_speed = 1.0
...
async def create_timed_dub_mp3(
    ...
    """Create final Khmer dub MP3 by synthesizing and mixing all cues."""
    ...
    logger.info("Final MP3 created: %s bytes", output_path.stat().st_size)
...
async def process_srt_upload(
    ...
    """Process uploaded SRT file: translate and synthesize."""
...
async def process_media(
    ...
    """Process media file: extract audio, transcribe, translate, synthesize."""
    status = await update.message.reply_text("⏳ កំពុងចាប់ផ្ដើម… 0%")
    track_message(status)
    state = {"last_percent": -10}
    ...
    try:
        ...
        await progress(12, "កំពុងស្គាល់សំឡេង")
...
async def handle_audio(
    ...
    if media.duration and media.duration > MAX_MEDIA_SECONDS:
        message = await update.message.reply_text(
            f"សំឡេងវែងជាង {MAX_MEDIA_SECONDS} វិនាទី។ សូមកាត់ឱ្យខ្លីជាងនេះ។"
        )
        track_message(message)
        session["processing_task"] = None
        return
...
async def handle_video(
    ...
    if video.duration and video.duration > MAX_MEDIA_SECONDS:
        message = await update.message.reply_text(
            f"វីដេអូវែងជាង {MAX_MEDIA_SECONDS} វិនាទី។ "
            f"សូមកាត់ឱ្យនៅត្រឹម {MAX_MEDIA_SECONDS} វិនាទី ឬតិចជាងនេះ។"
        )
        track_message(message)
        session["processing_task"] = None
        return
...
# ==============================================================================
# Bot Initialization and Entry Point
# ==============================================================================
...
def main() -> None:
    ...
    logger.info(
        "KhmerDubAI Turbo Server started | max=%ss | idle=%ss | whisper=%s | parallel=%s",
        MAX_MEDIA_SECONDS,
